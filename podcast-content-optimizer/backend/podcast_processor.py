...
from utils import get_podcast_episodes, download_episode, run_with_animation, save_processed_podcast, file_path_to_url, safe_filename, get_episode_folder, upload_to_firebase, PROCESSED_PODCASTS_FILE
...
def process_podcast_episode(rss_url, episode_index=0, job_id=None):
    ...
    try:
        ...
        if not os.path.exists(os.path.join(episode_folder, input_filename)):
            ...
            podcast_data['input_file'] = os.path.join(episode_folder, input_filename)
            logging.info(f"Downloaded input file: {podcast_data['input_file']}")
        else:
            logging.info(f"Using existing audio file: {os.path.join(episode_folder, input_filename)}")
            podcast_data['input_file'] = os.path.join(episode_folder, input_filename)

        # Upload the input file to Firebase and update the URL
        podcast_data['input_file'] = upload_to_firebase(podcast_data['input_file'])
        logging.info(f"Uploaded input file to Firebase: {podcast_data['input_file']}")
        save_processed_podcast(podcast_data)

        if not os.path.exists(os.path.join(episode_folder, input_filename)):
            raise FileNotFoundError(f"Downloaded file not found: {os.path.join(episode_folder, input_filename)}")

        logging.info(f"File size of downloaded episode: {os.path.getsize(os.path.join(episode_folder, input_filename))} bytes")
        ...
        if not os.path.exists(os.path.join(episode_folder, transcript_filename)):
            ...
            logging.info(f"Using existing transcript file: {os.path.join(episode_folder, transcript_filename)}")
            podcast_data['transcript_file'] = upload_to_firebase(os.path.join(episode_folder, transcript_filename))
            logging.info(f"Uploaded existing transcript file to Firebase: {podcast_data['transcript_file']}")
            save_processed_podcast(podcast_data)
        ...
        try:
            if unwanted_content['unwanted_content']:
                ...
                podcast_data['output_file'] = upload_to_firebase(os.path.join(episode_folder, input_filename))
            podcast_data['status'] = 'edited'
            save_processed_podcast(podcast_data)
        except Exception as e:
            logging.error(f"Error during audio editing: {str(e)}")
            output_file = os.path.join(episode_folder, input_filename)
            podcast_data['output_file'] = upload_to_firebase(os.path.join(episode_folder, input_filename))
        ...
        if os.path.exists(os.path.join(episode_folder, output_file)):
            result["edited_url"] = file_path_to_url(os.path.join(episode_folder, output_file))
        if os.path.exists(os.path.join(episode_folder, transcript_filename)):
            result["transcript_file"] = file_path_to_url(os.path.join(episode_folder, transcript_filename))
        if os.path.exists(os.path.join(episode_folder, unwanted_content_filename)):
            result["unwanted_content_file"] = file_path_to_url(os.path.join(episode_folder, unwanted_content_filename))
